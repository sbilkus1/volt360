...
void AChimeraHUD::DrawMainHUD(float W, float H)
{
	...
	// GDD 7 — phone notification indicator.
	if (auto* PH = GetGameInstance()->GetSubsystem<UPhoneSystem>())
	{
		int32 UR = PH->UnreadCount();
		if (UR > 0)
		{
			Canvas->DrawText(F, FString::Printf(TEXT("  [PHONE: %d new]"), UR),
				FVector2D(W - 180, 60), FVector2D(0.9f, 0.9f), FLinearColor(0.3f, 1.f, 0.5f));
		}
	}

	// GDD 7 — radio station indicator.
	if (auto* RS = GetGameInstance()->GetSubsystem<URadioSystem>())
	{
		Canvas->DrawText(F, FString::Printf(TEXT("RADIO: %s"), *RS->GetStations()[RS->GetStation()].Name),
			FVector2D(W - 250, H - 24), FVector2D(0.8f, 0.8f), FLinearColor(0.6f, 0.8f, 1.f));
	}
